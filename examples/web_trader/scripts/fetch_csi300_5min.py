"""Download CSI 300 5-minute bars via AkShare and cache them locally."""
...
CACHE = Path(__file__).with_name("csi300_5min_cache.json")
...
def fetch_sina_5min():
    ...
    return ak.stock_zh_a_minute(symbol="sh000300", period="5", adjust="")
...
def main() -> None:
    ...
    df = None
    try:
        df = fetch_eastmoney_5min()
        source = "akshare.index_zh_a_hist_min_em 000300 period=5"
    except Exception as exc:
        print(f"eastmoney skip: {type(exc).__name__} {exc}")

    if df is None or df.empty:
        df = fetch_sina_5min()
        source = "akshare.stock_zh_a_minute sh000300 period=5"

    if df is None or df.empty:
        raise RuntimeError("AkShare 未返回沪深300 5分钟数据")
