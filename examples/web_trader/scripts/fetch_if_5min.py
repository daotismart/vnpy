"""Download IF (CFFEX CSI 300 futures) 5-minute bars: Eastmoney 主连 + Sina 分月合约."""
...
CACHE = Path(__file__).with_name("if_5min_cache.json")
...
def if_month_codes() -> list[str]:
    codes = ["IF0"]
    year, month = 2019, 12
    while (year, month) <= (2026, 9):
        codes.append(f"IF{year % 100:02d}{month:02d}")
        if month == 12:
            year, month = year + 1, 1
        else:
            month += 1
...
def main() -> None:
    ...
    try:
        ...
        merge_rows(bag, em)
        sources.append(f"eastmoney IF/沪深300主连 5min x{len(em)}")
    ...
    merge_rows(bag, sina)
    sources.append(f"sina IF months 5min x{len(sina)}")
    print(f"sina {len(sina)}")
    rows = [bag[key] for key in sorted(bag)]
    if len(rows) < 80:
        raise RuntimeError("未拿到足够的 IF 5 分钟线")
