"""Download SA 30-minute bars: resample 5min cache + Sina monthly + Eastmoney 主连."""
...
CACHE = Path(__file__).with_name("sa_30min_cache.json")
CACHE_5MIN = Path(__file__).with_name("sa_5min_cache.json")
...
def ceil_30min(dt: datetime) -> datetime:
    """5-minute bars are labeled at bar end; map into the enclosing 30-minute close."""
...
def snap_rows(rows: list[list]) -> list[list]:
    buckets: dict[str, list] = {}
    for raw in rows:
        row = sanitize_bar(raw)
        if row is None:
            continue
        key = ceil_30min(parse_dt(str(row[0]))).strftime("%Y-%m-%d %H:%M:%S")
        oi, hi, lo, close = float(row[1]), float(row[2]), float(row[3]), float(row[4])
        vol = float(row[5] or 0)
        old = buckets.get(key)
        if old is None:
            buckets[key] = [key, oi, hi, lo, close, vol]
        else:
            old[2] = max(old[2], hi)
            old[3] = min(old[3], lo)
            old[4] = close
            old[5] = float(old[5]) + vol
    return [buckets[k] for k in sorted(buckets)]


def resample_5min() -> list[list]:
    if not CACHE_5MIN.exists() or CACHE_5MIN.stat().st_size < 100:
        return []
    raw = json.loads(CACHE_5MIN.read_text(encoding="utf-8"))
    return snap_rows(raw)
...
def main() -> None:
    ...
    bag: dict[str, list] = {}
    sources: list[str] = []
    resampled = resample_5min()
    merge_rows(bag, resampled)
    sources.append(f"resample 5min→30min x{len(resampled)}")
    print(f"resample {len(resampled)}")
    sina = snap_rows(fetch_sina_months())
    merge_rows(bag, sina)
    sources.append(f"sina SA months 30min x{len(sina)}")
    print(f"sina {len(sina)}")
    try:
        em = snap_rows(fetch_eastmoney_main())
        merge_rows(bag, em)
        sources.append(f"eastmoney 纯碱主连 30min x{len(em)}")
    ...
    if len(rows) < 80:
        raise RuntimeError("未拿到足够的 SA 30 分钟线")
