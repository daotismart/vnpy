"""Download SA (CZCE soda ash) dominant-contract daily bars via AkShare."""
...
CACHE = Path(__file__).with_name("sa_daily_cache.json")
...
def fetch() -> tuple[object, str]:
    ...
    errors: list[str] = []
    attempts = [
        ("futures_main_sina SA0", lambda: ak.futures_main_sina(symbol="SA0")),
        ("futures_zh_daily_sina SA0", lambda: ak.futures_zh_daily_sina(symbol="SA0")),
        ("futures_hist_em SA0", lambda: ak.futures_hist_em(symbol="SA0", period="daily")),
    ]
    ...
    raise RuntimeError("AkShare 未返回 SA 日线: " + " | ".join(errors))
...
def main() -> None:
    ...
    if len(rows) < 80:
        raise RuntimeError(f"SA 日线样本不足: {len(rows)} 来源 {source}")
